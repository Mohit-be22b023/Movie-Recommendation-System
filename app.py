...
# Bump this whenever app.py changes. It is shown in the sidebar so you can tell
# at a glance which copy is actually deployed — the usual cause of "but I fixed
# that already" is an older file still sitting in the repo.
APP_VERSION = "1.4.1"
...
# Embedded artifacts
# ═══════════════════════════════════════════════════════════════════════════
# The trained embeddings and the film catalogue are stored in this file as
# zlib-compressed base64, so `app.py` alone is a complete deployment. Files in
# models/ still win when present, which keeps retraining meaningful: run
# `python app.py --embed` after training to refresh the payload below.
#
# float32 is kept deliberately. float16 halves the payload but reorders 17% of
# top-10 lists and int8 reorders 80% of them, so the saving is not worth it.
_EMBEDDED_SHAPE = ()
_EMBEDDED_EMBEDDINGS = ""
_EMBEDDED_CATALOG = ""


def has_embedded_artifacts() -> bool:
    return bool(_EMBEDDED_EMBEDDINGS and _EMBEDDED_CATALOG and _EMBEDDED_SHAPE)


def load_embedded_artifacts():
    """Decode the payload baked into this file."""
    raw = zlib.decompress(base64.b64decode(_EMBEDDED_EMBEDDINGS))
    embeddings = np.frombuffer(raw, dtype=np.float32).reshape(_EMBEDDED_SHAPE).copy()
    catalog_csv = zlib.decompress(base64.b64decode(_EMBEDDED_CATALOG)).decode("utf-8")
    return embeddings, pd.read_csv(io.StringIO(catalog_csv))


def embed_artifacts_into_self(models_dir: "str | Path" = "models") -> None:
    """
    `python app.py --embed` -- rewrite this file's payload from models/.

    Run it after training so the single-file app carries the newest embeddings.
    """
    import pickle as _pickle

    models_dir = Path(models_dir)
    embeddings = np.load(models_dir / "movie_embeddings.npy").astype(np.float32)
    with open(models_dir / "movie_id_mapping.pkl", "rb") as fh:
        catalog = _pickle.load(fh)["catalog"]

    emb_blob = base64.b64encode(zlib.compress(embeddings.tobytes(), 9)).decode()
    cat_blob = base64.b64encode(
        zlib.compress(catalog.to_csv(index=False).encode("utf-8"), 9)).decode()

    source = Path(__file__).read_text(encoding="utf-8")
    source = re.sub(r"^_EMBEDDED_SHAPE = .*$",
                    f"_EMBEDDED_SHAPE = {tuple(embeddings.shape)}", source, count=1, flags=re.M)
    source = re.sub(r"^_EMBEDDED_EMBEDDINGS = .*$",
                    f'_EMBEDDED_EMBEDDINGS = "{emb_blob}"', source, count=1, flags=re.M)
    source = re.sub(r"^_EMBEDDED_CATALOG = .*$",
                    f'_EMBEDDED_CATALOG = "{cat_blob}"', source, count=1, flags=re.M)
    Path(__file__).write_text(source, encoding="utf-8")
    size = Path(__file__).stat().st_size / 1e6
    print(f"Embedded {embeddings.shape[0]:,} x {embeddings.shape[1]} embeddings and "
          f"{len(catalog):,} catalogue rows.")
    print(f"app.py is now {size:.2f} MB and needs no other files to run.")


if "--version" in sys.argv:        # `python app.py --version`
    print(f"CineLatent app.py v{APP_VERSION} ({APP_BUILD})")
    print("embedded artifacts: " +
          (f"{_EMBEDDED_SHAPE[0]:,} x {_EMBEDDED_SHAPE[1]}" if _EMBEDDED_SHAPE else "none"))
    sys.exit(0)


if "--embed" in sys.argv:          # CLI path; Streamlit never passes this flag
    embed_artifacts_into_self(sys.argv[sys.argv.index("--embed") + 1]
                              if len(sys.argv) > sys.argv.index("--embed") + 1 else "models")
    sys.exit(0)
...
class MovieRecommender:
    ...
    MAPPING_FILE = "movie_id_mapping.pkl"

    @classmethod
    def candidate_dirs(cls, models_dir=None) -> List[Path]:
        """Where the artifacts might live, in priority order."""
        dirs = [Path(models_dir)] if models_dir else []
        dirs += [MODELS_DIR, ROOT / "models", ROOT]
        seen, unique = set(), []
    ...
    @classmethod
    def load(cls, models_dir=None, **kwargs) -> "MovieRecommender":
        """
        Load the saved artifacts. `models_dir` may be a str or a Path; if the
        files are not there, ./models and the repo root are tried too, so the
        app still starts when the artifacts sit beside app.py.
        """
        searched = cls.candidate_dirs(models_dir)
        emb_path = map_path = None
        for directory in searched:
            candidate_emb = directory / cls.EMBEDDINGS_FILE
            candidate_map = directory / cls.MAPPING_FILE
            if candidate_emb.exists() and candidate_map.exists():
                emb_path, map_path = candidate_emb, candidate_map
                break
        if emb_path is None:
            if has_embedded_artifacts():
                embeddings, catalog = load_embedded_artifacts()
                instance = cls(embeddings, catalog, **kwargs)
                instance.source = "embedded in app.py"
                return instance
            locations = "\n  ".join(str(d) for d in searched)
            raise FileNotFoundError(
                f"Could not find {cls.EMBEDDINGS_FILE} and {cls.MAPPING_FILE}. "
                f"Looked in:\n  {locations}"
            )
        ...
        instance = cls(embeddings, catalog, **kwargs)
        instance.source = str(emb_path.parent)
        return instance
...
class TMDBClient:
    ...
    def _redact(self, text) -> str:
        """
        Strip the credential out of anything that might be displayed or logged.

        A v3 API key travels in the URL query string, so a proxy, gateway or
        WAF error page can echo the whole URL back -- key included. Redacting
        here means the key cannot reach the Streamlit UI or the app logs even
        when an intermediary misbehaves.
        """
        if not text:
            return ""
        return str(text).replace(self._credential, "***REDACTED***")
    ...
    def _get(self, endpoint: str, params: Optional[Dict[str, Any]] = None) -> Optional[dict]:
        ...
        if response.status_code in (401, 403):
            # Surface TMDB's own explanation ("Invalid API key: ...") -- and, if
            # the response is not TMDB JSON at all, the raw body, which is how a
            # blocking proxy or firewall reveals itself instead of masquerading
            # as a bad key.
            try:
                hint = str((response.json() or {}).get("status_message", "")).strip()
            except ValueError:
                hint = str(getattr(response, "text", ""))[:140].strip()
            hint = self._redact(hint)
            raise TMDBAuthError(
                f"TMDB rejected the credential (HTTP {response.status_code})."
                + (f" {hint}" if hint else "")
            )
...
def load_recommender(models_dir: str, app_version: str = APP_VERSION) -> MovieRecommender:
    """
    `app_version` is part of the cache key on purpose.

    Streamlit Cloud reuses the Python process when the code changes, and
    @st.cache_resource survives that. Without the version in the key, new code
    is handed an object built by the previous version of this class -- which
    fails with an AttributeError on whatever attribute was just added.
    """
    return MovieRecommender.load(models_dir)


@st.cache_resource(show_spinner=False)
def load_tmdb_client(app_version: str = APP_VERSION) -> Tuple[Optional[TMDBClient], str, str]:
    """
    Resolve the TMDB credential and say what happened.

    Returns (client, status, detail) where status is one of "ok",
    "no-credential", "bad-credential" or "requests-missing". Returning the
    reason matters: silently handing back None is what makes an app show grey
    placeholder posters with no explanation of why.
    """
    ...
    try:
        ...
        pass  # no secrets configured at all — the ordinary case, not an error
    except Exception as exc:
        # A secrets file that exists but does not parse. Almost always an
        # unquoted value: TOML requires TMDB_API_KEY = "abc123", quotes included.
        # Reporting this separately matters — otherwise a broken secrets file is
        # indistinguishable from having set no secret at all.
        notes.append(
            f"Streamlit secrets exist but could not be read ({type(exc).__name__}). "
            "Check the value is quoted and at the top level, not under a [section]."
        )
    ...
    if not credential:
        ...
        try:
            ...
            if env_file.exists():
                notes.append(
                    "A .env file is present but python-dotenv is not installed, "
                    "so it was skipped. Add python-dotenv to requirements.txt."
                )
    ...
    if not credential:
        if notes:
            return None, "credential-unreadable", " ".join(notes)
        return None, "no-credential", "No TMDB_API_KEY in Streamlit secrets, the environment or .env."
    try:
        client = TMDBClient(credential)
    except ImportError:
        return None, "requests-missing", "The `requests` package is not installed."
    except TMDBAuthError as exc:
        return None, "bad-credential", str(exc)
    style = "v4 bearer token" if client._is_bearer else "v3 API key"
    return client, "ok", f"{style} loaded from {source}."


def probe_tmdb() -> Tuple[bool, str]:
    """One live lookup, so the sidebar can prove the credential really works."""
    client, status, detail = load_tmdb_client()
    if client is None:
        return False, detail
    try:
        hit = client.search_movie("Toy Story", 1995)
    except TMDBAuthError as exc:
        return False, f"TMDB rejected the credential. {exc}"
    except TMDBError as exc:
        return False, f"Could not reach TMDB. {exc}"
    if not hit:
        return False, "TMDB answered but returned no results for the test query."
    poster = get_poster_url(hit.get("poster_path"))
    if not poster:
        return False, "TMDB matched the film but it has no poster image."
    return True, f"Connected. Test lookup returned TMDB id {hit['id']} and a poster URL."
...
def fetch_metadata(title: str) -> dict:
    ...
    client, status, detail = load_tmdb_client()
    if client is None:
        return {"tmdb_found": False, "tmdb_error": detail}
    try:
...
def render_results(recs: pd.DataFrame, use_tmdb: bool, columns: int = 5) -> None:
    ...
    if use_tmdb:
        ...
        if failed and len(failed) == len(recs):
            reasons = {str(m.get("tmdb_error")) for m in metas.values() if m.get("tmdb_error")}
            st.warning(
                "No posters: every TMDB lookup failed, so MovieLens metadata is shown instead."
                + (f"  Reason: {' | '.join(sorted(reasons))}" if reasons else ""),
                icon="🖼️",
            )
        elif failed:
...
try:
    rec = load_recommender(str(MODELS_DIR), APP_VERSION)
except FileNotFoundError as exc:
    show_missing_artifacts(exc)
    st.stop()
except (pickle.UnpicklingError, EOFError, ModuleNotFoundError) as exc:
    # Only genuine unpickling failures land here. An earlier version also caught
    # AttributeError, which meant an ordinary bug was reported as a pandas
    # version mismatch and sent debugging in the wrong direction.
    st.error(f"The saved catalogue could not be unpickled — {type(exc).__name__}: {exc}")
    st.info(
    ...
    # Almost certainly a cached object from an older deploy. Clear and retry once.
    load_recommender.clear()
    try:
        rec = load_recommender(str(MODELS_DIR), APP_VERSION)
    except Exception as retry_exc:
        st.error(f"Could not load the saved artifacts — "
                 f"{type(retry_exc).__name__}: {retry_exc}")
        st.exception(retry_exc)
        st.stop()
except Exception as exc:
    st.error(f"Could not load the saved artifacts — {type(exc).__name__}: {exc}")
    st.caption("Full traceback below. This is the real error, not a guess at the cause.")
    st.exception(exc)
    st.stop()

tmdb_client, tmdb_status, tmdb_detail = load_tmdb_client()
...
with st.sidebar:
    ...
    # Posters are the first thing people notice missing, so the reason is always
    # on screen — not hidden behind a toggle the user never switches on.
    st.markdown("**Posters (TMDB)**")
    if tmdb_status == "ok":
        st.success(tmdb_detail, icon="🔑")
    elif tmdb_status == "no-credential":
        st.warning(
            "No API key, so posters fall back to a grey placeholder.\n\n"
            "**Streamlit Cloud** — Manage app → Settings → Secrets, paste this "
            "exactly (the quotes are required, and it must not sit under a "
            "`[section]` heading), then Reboot:\n\n"
            "```toml\nTMDB_API_KEY = \"your_key_here\"\n```\n"
            "**Locally** — create a file named `.env` beside `app.py` containing "
            "`TMDB_API_KEY=your_key_here` (no quotes in .env), then restart.\n\n"
            "Free key: themoviedb.org → Settings → API. After adding it, press "
            "**Check TMDB connection** below — the credential is cached at "
            "startup, so a key added since then is not picked up until you do.",
            icon="🖼️",
        )
        want_tmdb = False
    elif tmdb_status == "requests-missing":
        st.error("`requests` is not installed — add it to requirements.txt.", icon="📦")
        want_tmdb = False
    else:
        st.error(f"TMDB credential problem: {tmdb_detail}", icon="🔑")
        want_tmdb = False

    if st.button("Check TMDB connection", use_container_width=True):
        load_tmdb_client.clear()      # pick up a key added since the app started
        fetch_metadata.clear()
        ok, message = probe_tmdb()
        (st.success if ok else st.error)(message)
        if ok:
            st.caption("Reload the page if posters are still not showing.")

    st.divider()
    st.caption(
        f"{len(rec.catalog):,} movies · {rec.embeddings.shape[1]}-dimensional embeddings\n\n"
        f"Artifacts: {getattr(rec, 'source', 'unknown')} · app v{APP_VERSION}\n\n"
